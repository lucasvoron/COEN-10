...
int main()
{
	...
	while(1)//forever loop
	{
		...
		printf("Press 6 to create a priority appointment\n");
		printf("Press 7 to search to number of occurences of a letter in the schedule\n");
...
void priority(int flag)
{
	...
	if(flag==0)
	{
		printf("Please enter your information for this priority appointment:\n");
		printf("Please enter your name.\n");
		scanf(" %s", name);
		...
		printf("Your appointment for %s has been set at 1pm.\n", name);

		printf("%s's (%s) appointment has been cancelled.\n", schedule[4], phoneNumbers[i]);
		i = 4;
		while(i>0)
		{
			strcpy(schedule[i], schedule[i-1]);
		...
		printf("A priority appointment has already been scheduled.\n");
}
